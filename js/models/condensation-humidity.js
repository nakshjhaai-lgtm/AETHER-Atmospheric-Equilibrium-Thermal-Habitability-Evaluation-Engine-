...
export function calculateCondensation({
  ...
  if (cooling_rate_k_per_s < 0) {
    ...
    if (current_vapor > new_es) {
      ...
      const temperature_rise = latent_heat / (1005 * 1.225); // simplified heating: assumes dry air density. Upgrade: use actual atmospheric column density.
...
export function calculateCloudFormation({
  ...
  if (condensation_rate > 0 && humidity > 0.95) {
    ...
    // Cloud optical depth
    const liquid_water_path = condensation_rate * 1000; // simplified LWP: does not account for droplet size distribution or altitude. Upgrade: use a cloud microphysics parameterization.
    results.cloud_optical_depth = liquid_water_path * 0.15; // approximate cloud OD: not validated against observations. Upgrade: use a two-stream cloud optical model.
    
    // simplified cloud fraction: no overlap or vertical distribution. Upgrade: use a statistical cloud scheme.
...
export function calculateHumidityProfile(temperature_profile, surface_humidity) {
  return temperature_profile.map((level) => {
    ...
    return {
      ...
      dew_point_k: level.temperature_k - (1 - humidity) * 50 // simplified dew point: rough linear approximation. Upgrade: use Magnus formula with actual vapor pressure.
    };
  });
}
