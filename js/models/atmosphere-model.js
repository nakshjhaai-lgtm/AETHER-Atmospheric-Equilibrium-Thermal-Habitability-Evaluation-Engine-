...
import { GAS_PROPERTIES, ATMOSPHERE_PRESETS } from '../schema/constants.js';
import { computeGasOpticalDepth } from './gas-opacity.js';
...
export class AtmosphereModel {
  ...
  // Mean opacity for reduced model
  // Priority: legacy optical depth > gas-composition-derived > rough estimate
  getReducedGreenhouseOpacity(temperatureK) {
    // Use legacy optical depth if explicitly set (from presets or user input)
    if (this.legacyOpticalDepth !== null) return this.legacyOpticalDepth;

    // Use gas-composition-derived opacity if gases are available
    const tempK = temperatureK ?? 288;
    const gasOpacity = this.computeGasDerivedOpacity(tempK);
    if (gasOpacity.totalTau > 0) return gasOpacity.totalTau;

    // Fallback: rough estimate from known greenhouse gases
    return this._estimateOpacityFromComposition();
  }

  // Gas-composition-derived opacity using lookup tables
  computeGasDerivedOpacity(temperatureK) {
    return computeGasOpticalDepth(this.gasMixingRatios, this.totalPressureBar, temperatureK);
  }

  _estimateOpacityFromComposition() {
    ...
    let tau = 0.1;
    tau += co2 * 50 * Math.log(1 + this.totalPressureBar);
    tau += ch4 * 200;
    tau += h2o * 10 * this.humidity;
    tau += n2o * 300;
    tau *= Math.sqrt(this.totalPressureBar);
