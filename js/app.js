...
window.addEventListener('DOMContentLoaded', () => {
  ...
  bindDisclaimer(); bindOnboarding(); bindReset(); bindSave(); bindShare(); bindEscapeKey(); bindOfflineState();
  ...
  if (tabBar) {
    ...
    tabBar.addEventListener('pointerup', (e) => {
      if (started && isMobile()) {
        ...
        if (dy < 10) {
          // Tap (not swipe) — toggle dock
          state.ui.mobileDockOpen = !state.ui.mobileDockOpen;
          refs.controlPanel.classList.toggle('is-docked', state.ui.mobileDockOpen);
        }
      }
      started = false;
    });
...
function bindSave() {
  ...
  btn.addEventListener('click', () => {
    const data = {
      star: { preset: state.star.preset, teff: state.star.teff, rstar: state.star.rstar },
      planet: {
        distance: state.planet.distance, radius: state.planet.radius, mass: state.planet.mass,
        albedo: state.planet.albedo, tau: state.planet.tau, core: state.planet.core,
        atmoGases: state.planet.atmoGases, surfacePressureBar: state.planet.surfacePressureBar, atmoPreset: state.planet.atmoPreset
      },
      biologyTarget: currentBiologyTarget,
      fidelity: currentFidelity,
      savedAt: new Date().toISOString()
    };
    try {
      localStorage.setItem('aether:saved-config', JSON.stringify(data));
      btn.style.borderColor = 'var(--cyan)';
      btn.title = 'Configuration saved!';
      setTimeout(() => { btn.style.borderColor = ''; btn.title = 'Save current configuration'; }, 2000);
    } catch (e) {
      btn.title = 'Save failed: storage full';
    }
  });
  // Load saved config if exists
  try {
    const saved = JSON.parse(localStorage.getItem('aether:saved-config') || 'null');
    if (saved && saved.star) {
      // Don't auto-load, just keep it available
    }
  } catch (_) {}
}
